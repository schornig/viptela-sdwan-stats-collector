...
class vManageStatsCollector(object):
    ...
    def RealTimeAPICall(self, query_data=None):
        ...
        URLS = ['{}?deviceId={}&&&'.format(query_data['url_endpoint'], device_id) for device_id in query_data['deviceIDs']]

        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            future_to_measurement = {executor.submit(self.vManageSession.get_request, url): url for url in URLS}
            for future in concurrent.futures.as_completed(future_to_measurement):
                measurement =  future_to_measurement[future]
                ind_data = future.result()
                if 'data' in ind_data:
                    if data:
...
def TaskScheduller(vm=None, db=None, measurements=None):
    ...
    for collect_interval, measurements_list in tsort_measurements.items():
        ...
        if current_time_sec % collect_interval == 0:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                future_to_measurement = {executor.submit(getattr(vm, entry['query_type']), entry['query_data']): entry for entry in measurements_list}
